...
public class SimpleConsumer {
    ...
    // 컨슈머가 중단되거나 재시작되더라도 컨슈머 그룹의 '컨슈머 오프셋'을 기준으로 이후 데이터를 처리하기 때문
    ...
    public static void main(String[] args) {
        ...
        consumer.subscribe(List.of(TOPIC_NAME));//컨슈머에게 토픽을 할당하기 위해 subscribe()메서드 사용. 이 메서드는 Collection 타입의 String 값들을 인자로 받는데, 1개 이상의 토픽 이름을 받을 수 있다.
        while(true){
            ...
            // poll() 메서드는 Duration 타입을 인자로 받는다. 이 인자 값은 브로커로부터 데이터를 가져올 때 컨슈머 버퍼에 데이터를 기다리기 위한 타임아웃 간격을 뜻한다.
