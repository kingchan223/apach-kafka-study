...
//Consumer가 실행될 스레드를 정의하기 위해 Runnable 인터페이스로 ConsumerWoker클래스 구현
//ConsumerWoker클래스를 스레드로 실행하면 오버라이드된 run()메서드가 수행됨.
public class ConsumerWorker implements Runnable {

    private final Logger logger = LoggerFactory.getLogger(ConsumerWorker.class);

    //컨슈머의 poll메서드를 통해 전달받은 데이터를 임시저장하는 버퍼. 다수 스레드가 접근할 수 있으므로 ConcurrentHashMap 사용
    //bufferString에는 파티션 번호와 메시지 값들이 들어간다.
    private static Map<Integer, List<String>> bufferString = new ConcurrentHashMap<>();
    // currentFileOffset은 오프셋값을 저장하고 파일 이름을 저장할 때 오프셋 번호를 붙이는 데에 사용된다.
    ...
    public ConsumerWorker(Properties prop, String topic, int number) {
        ...
        this.prop = prop;
        this.topic = topic;
        this.threadName = "consumer-thread-" + number;//스레드에 이름을 붙ㅇ 로깅시에 편하게 함.
    }
    ...
    public void run() {
        ...
        consumer = new KafkaConsumer<>(prop);//스레드를 생성하는 HdfsSinkApplication에서 설정한 컨슈머 설정을 가져와서
        consumer.subscribe(Arrays.asList(topic));//KafkaConsumer 인스턴스를 생성하고 토픽을 구독
        ...
        try {
            while (true) {
                ...
                // 한번 polling이 완료되고 버퍼에 쌓임 ==>
                // saveBufferToHdfsFile을 호출해서 버퍼에 쌓인 데이터 일정 개수 도달 ==>
                // HDFS에 저장
    ...
    private void addHdfsFileBuffer(ConsumerRecord<String, String> record) {
        ...
        if (buffer.size() == 1)//만약 버퍼의 크기가 1이라면 버퍼의 가장 처음 오프셋이라는 뜻이므로 currentFileOffset변수에 넣는다.
    ...
    //버퍼의 데이터가 flush될 만큼 개수가 충족되었는지 확인하는 checkFlushCount 메서드 호출.
    ...
    private void save(int partitionNo) {
        if (bufferString.get(partitionNo).size() > 0)
            try {
                //HDFS에 저장할 파일 이름. 파티변 번호, 오프셋 번호ㅇ를 파일 이름을 보고 알 수 있도록 정함
                ...
                // bufferString에 쎃인 데이터를 fileOuputStream에 저장. 저장이 완료된 이후에는 close()메서드를 통해 안전하게 종료
                ...
                //버퍼 데이터가 적재 완료되었다면 새로 ArrayList를 선언하여 버퍼 데이터를 초기화
                bufferString.put(partitionNo, new ArrayList<>());
            } catch (Exception e) {

                // HDFS 파일 적재시 Exception이 발생하면 에러 로그를 남긴다.
                logger.error(e.getMessage(), e);
            }
    }
